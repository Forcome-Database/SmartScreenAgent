...
**Status:** In progress - the [WP4 design](2026-07-21-wp4-read-apis-design.md) and [implementation plan](../plans/2026-07-21-wp4-read-apis.md) cover the JD-scoped ranked candidate list, the flat candidate list, candidate detail (audited PII decrypt), score detail with evidence, an audited raw-file presigned download, JD list/detail, and rule-version list/diff. All nine routes are implemented, response-model-typed, and role-protected; the OpenAPI contract test asserts every route is present with a `200` response. Local evidence on 2026-07-21: offline suite 260 passed; the full integration suite (including the new OpenAPI contract test and the strengthened rule-version `is_active`/ordering/unknown-version-404 assertions) 79 passed against real PostgreSQL/Redis/MinIO; Ruff and mypy clean for 79 application source files. WP4 is not yet Complete: hosted CI has not run against this branch. WP4 will be marked Complete, and WP5 changed to Ready for planning, only after hosted CI passes.

...
| Rule editor and version workflow | Original W3-W4 | Import-only CLI; active version pointer exists; WP4 adds read-only JD list/detail, rule-version list (with `is_active`), and a structured version diff (`GET /jds/{code}/rule-versions/{from_version}/diff/{to_version}`). Controlled rule publication (a write workflow) remains unimplemented (WP6) | Import tests; WP4 integration tests cover JD list/detail, rule-version `is_active` flag and `published_at` DESC ordering, diff changes, and 404 for an unknown JD or version | Partial (read surface Complete under WP4, hosted CI run 29813134361 green; publication workflow still Absent, deferred to WP6) |

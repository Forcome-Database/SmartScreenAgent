...
| Candidate list and scorecard | Original W3-W5 | WP4: candidate detail (`GET /candidates/{id}`, decrypts PII under exactly one `pii_decrypt` audit row), score detail with evidence (`GET /candidates/{id}/scores/{score_id}`), and an audited raw-file presigned download (`GET /candidates/{id}/raw-file`); no UI yet (WP5) | Integration tests cover PII decrypt plus exactly-one-audit-row, score-not-owned-by-candidate 404, presigned URL plus one `raw_file_access` audit row, object-missing `404`, and `503` on storage unavailability | Implemented (API only; WP4 Complete, hosted CI run 29813134361 green) |
...
**Status:** In progress - the [WP5 design](2026-07-21-wp5-hr-web-workspace-design.md) and [implementation plan](../plans/2026-07-21-wp5-hr-web-workspace.md) Tasks 1-8 are implemented: a Next.js 15 App Router BFF frontend (`frontend/`) with DingTalk login handoff, an HMAC-SHA256-signed httpOnly session cookie, a server-side `/api/proxy/[...path]` route that attaches the Bearer JWT (never exposed to the browser), candidate list (flat and JD-ranked), candidate detail (audited PII decrypt), scorecard with judge evidence, JD detail, upload with job/batch status polling, and an audited raw-file presigned-URL redirect. A Playwright golden-path e2e drives candidate list -> candidate detail -> scorecard against a stubbed BFF and asserts no Bearer token or presigned-URL text leaks into the DOM, and an `@axe-core/playwright` check asserts zero serious/critical accessibility violations on the candidate list; both run against desktop and mobile-width Playwright projects. A production Docker image (`frontend/Dockerfile`, two-stage `node:22-alpine` build using `next.config.ts`'s `output: "standalone"`) and a `frontend` `docker-compose.yml` service were added. Local gate on 2026-07-22: lint and typecheck clean; Vitest 33 tests across 14 files passed; Playwright e2e 4 tests (golden-path + a11y, desktop + mobile) passed; `next build` succeeded; the Docker image built (via the classic, non-BuildKit builder - see completion evidence in the [implementation plan](../plans/2026-07-21-wp5-hr-web-workspace.md)) and served `/login` with `200`. This evidence is local only: hosted CI has not run for WP5, and Task 9 (push, hosted gate, and marking WP5 Complete / WP6 Ready for planning) has not started. WP5 is In progress.

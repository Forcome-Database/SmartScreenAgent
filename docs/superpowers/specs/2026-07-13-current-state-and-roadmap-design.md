...
| Candidate query API | P2 Task 14 title promised upload, score, and query | WP4: JD-scoped ranked candidate list (`GET /jds/{code}/candidates`) and flat candidate list (`GET /candidates`), both paginated, filtered, and PII-free | Integration tests cover ranked/flat list shape, no-PII, no-audit, and unknown-JD 404; the OpenAPI contract test asserts both routes | Implemented (WP4 Complete; hosted CI run 29813134361 green) |
...
| Candidate list and scorecard | Original W3-W5 | WP4: candidate detail (`GET /candidates/{id}`, decrypts PII under exactly one `pii_decrypt` audit row), score detail with evidence (`GET /candidates/{id}/scores/{score_id}`), and an audited raw-file presigned download (`GET /candidates/{id}/raw-file`); WP5 ships the HR web UI over these APIs (PII-free lists, audited candidate detail, scorecard with evidence, raw-file download) | Integration tests cover PII decrypt plus exactly-one-audit-row, score-not-owned-by-candidate 404, presigned URL plus one `raw_file_access` audit row, object-missing `404`, and `503` on storage unavailability; WP5 Vitest + Playwright golden-path/a11y e2e cover the UI | Implemented (API WP4 Complete, hosted CI run 29813134361 green; UI WP5 Complete, local gate + backend-CI-no-regression run 29880568935) |
| Rule editor and version workflow | Original W3-W4 | WP4 adds read-only JD/rule-version APIs and structured diffs; WP6c adds validated drafts, golden-set What-If evaluation, recorded regression gates, transactional publication, active-version switching, predecessor archival, and the role-gated `/jds/{code}/rules` UI | Integration tests cover read APIs, draft creation, PII-free What-If metrics, publication gates, active-version isolation, concurrent publication, and migration round trips; frontend component and desktop/mobile Playwright tests cover the full workflow | Complete (WP4 read surface and WP6c controlled publication workflow) |
...
**Status:** Complete - the [WP4 design](2026-07-21-wp4-read-apis-design.md) and [implementation plan](../plans/2026-07-21-wp4-read-apis.md) cover the JD-scoped ranked candidate list, the flat candidate list, candidate detail (audited PII decrypt), score detail with evidence, an audited raw-file presigned download, JD list/detail, and rule-version list/diff. All nine routes are implemented, response-model-typed, and role-protected; the OpenAPI contract test asserts every route is present with a `200` response. The raw-file endpoint verifies object existence before presigning so a missing object returns `404` and an unreachable object store returns `503`, neither leaving an audit row. Local evidence on 2026-07-21: offline suite 260 passed; the full integration suite (including the OpenAPI contract test, the strengthened rule-version `is_active`/ordering/unknown-version-404 assertions, the object-missing-404 and all-nine-routes-401 sweeps, and the presigned-URL-not-logged leak test) 90 passed against real PostgreSQL/Redis/MinIO; Ruff and mypy clean for 79 application source files. Hosted [GitHub Actions run 29813134361](https://github.com/Forcome-Database/SmartScreenAgent/actions/runs/29813134361) passed Python 3.10, Python 3.14, and strict integration at commit `24ce55d` (WP4 range `3db197c..24ce55d`, PR #4). WP4 is Complete and WP5 is Ready for planning.
...
**Status:** Complete - the [WP5 design](2026-07-21-wp5-hr-web-workspace-design.md) and [implementation plan](../plans/2026-07-21-wp5-hr-web-workspace.md) are implemented: a Next.js 15 App Router BFF frontend (`frontend/`) with DingTalk login handoff, an HMAC-SHA256-signed httpOnly session cookie, a server-side `/api/proxy/[...path]` route that attaches the Bearer JWT (never exposed to the browser) and a hardened path allowlist (`safeUpstreamPath` rejects `../` and encoded-slash traversal and re-checks the normalized prefix), candidate list (flat and JD-ranked, PII-free), candidate detail (the single audited PII-decrypt fetch), scorecard with hard-filter/rule/judge dimensions and evidence, upload with per-file job-status polling, re-score (per JD), and an audited raw-file presigned-URL redirect (URL kept server-side). Every FastAPI-bound route is `readSession`-gated and clears the session cookie on upstream 401; `getServerEnv()` fails closed on a missing/placeholder/short session secret; upstream fetches use `cache: "no-store"`; logout clears the React Query cache. A Playwright golden-path e2e drives candidate list -> candidate detail (PII) -> scorecard (evidence) against a stubbed BFF with a real minted signed cookie and asserts no Bearer token or presigned-URL text leaks into the DOM, plus an `@axe-core/playwright` check for zero serious/critical accessibility violations; both run on desktop and mobile-width projects. A production Docker image (`frontend/Dockerfile`, two-stage `node:22-alpine`, `output: "standalone"`) and a `frontend` `docker-compose.yml` service were added. Local gate on 2026-07-22: lint and typecheck clean; Vitest 34 tests across 14 files passed; Playwright e2e 4 tests (golden-path + a11y, desktop + mobile) passed; `next build` succeeded. WP5 changes no backend code — the backend [GitHub Actions run 29880568935](https://github.com/Forcome-Database/SmartScreenAgent/actions/runs/29880568935) on PR #5 confirmed no backend regression (Python 3.10, Python 3.14, strict integration green). A dedicated frontend hosted-CI job is not yet wired (a documented follow-up); the comprehensive local gate is the exit evidence. WP5 range `bae8068..639436a`, PR #5. WP5 is Complete and WP6 is Ready for planning.
...
**Status:** Complete. WP6 was decomposed into three independently shippable sub-projects: WP6a (feedback capture and minimal reporting), WP6b (golden set and baseline metrics), and WP6c (rule publication workflow, What-If, and regression gates, which depends on WP6b). **WP6a is Complete** - the [WP6a design](2026-07-22-wp6a-feedback-capture-design.md) and [implementation plan](../plans/2026-07-22-wp6a-feedback-capture.md) are implemented: a `PUT`/`GET` feedback API on each score (`decision` advance/reject/hold + reason, server-derived `ai_agreed` from `score.grade`, one row per `(score_id, reviewer_user_id)` via upsert, `422 feedback_reason_required` when a disagreement is submitted without a reason) and a PII-free `GET /api/v1/feedback/report` aggregate (overall/per-JD agreement rate, paginated disagreement list), all role-gated `hr`/`hr_lead`/`admin`; a small Alembic migration (`1e9b39dbf340`, revises `2f27938b430b`) adds the `(score_id, reviewer_user_id)` uniqueness constraint and a `decision` CHECK; the WP5 scorecard gained a `FeedbackPanel` and a new `/reports/feedback` report page. Local evidence on 2026-07-22: backend offline suite 264 passed; the full integration suite 96 passed against real PostgreSQL/Redis/MinIO; Ruff and mypy clean for 82 application source files; frontend lint/typecheck clean, Vitest 36 tests across 16 files, Playwright e2e 6 tests (3 specs including the new `feedback.spec.ts`, desktop + mobile projects) passed, `next build` succeeded. Hosted [GitHub Actions run 29899061689](https://github.com/Forcome-Database/SmartScreenAgent/actions/runs/29899061689) passed Python 3.10, Python 3.14, and strict integration on PR #6. WP6a is Complete.

**WP6b is Complete.** The [WP6b design](2026-07-22-wp6b-golden-set-baseline-metrics-design.md) and [implementation plan](../plans/2026-07-22-wp6b-golden-set-baseline-metrics.md) add a curated ground-truth golden set independent of WP6a's per-reviewer feedback: `POST /api/v1/golden-set/import` (role `hr_lead`/`admin`, multipart CSV of `candidate_id, jd_code, label`, per-row validation with a partial-success summary, upsert on `uq_golden_set_cand_jd`, capped by `GOLDEN_IMPORT_MAX_ROWS`), `GET /api/v1/golden-set` (paginated, PII-free list), and `GET /api/v1/golden-set/metrics` (live-computed confusion matrix, precision/recall/F1/accuracy overall and per-JD, `borderline` excluded, `uncovered` reported, most-recent-score selection), all role-gated `hr`/`hr_lead`/`admin` for reads; a small Alembic migration (`f412481450cf`, revises WP6a's `1e9b39dbf340`) adds a `label` CHECK constraint (`advance`/`reject`/`borderline`) to the existing `golden_set` table. The frontend gained a `/golden-set` import+list page (import control gated to `hr_lead`/`admin`) and a `/reports/baseline` metrics page, plus nav entries. Local gate on 2026-07-22: backend offline suite 268 passed; the full integration suite 101 passed against real PostgreSQL/Redis/MinIO; Ruff and mypy clean for 85 application source files; frontend lint/typecheck clean, Vitest 39 tests across 18 files, Playwright e2e 8 tests (4 specs including the new `golden-set.spec.ts`, desktop + mobile projects) passed, `next build` succeeded. Hosted [GitHub Actions run 29926081927](https://github.com/Forcome-Database/SmartScreenAgent/actions/runs/29926081927) passed Python 3.10, Python 3.14, and strict integration on PR #7. WP6b is Complete.

**WP6c and WP6 are Complete.** The [WP6c design](2026-07-23-wp6c-rule-publication-design.md) and [implementation plan](../plans/2026-07-23-wp6c-rule-publication.md) are implemented: rule versions now have `draft`/`published`/`archived` states and per-JD version uniqueness; curators can create validated drafts, run an offline What-If over the golden set using re-evaluated hard/rule dimensions plus the stored judge subtotal, compare draft metrics with the active baseline, and publish only after regression metrics are recorded. Publishing switches `JD.active_rule_version_id`, archives the predecessor, and records the publisher. The `/jds/{code}/rules` UI exposes the workflow with role-gated actions, approximation warnings when judge dimensions change, and no candidate PII in metric responses. Local gate on 2026-07-23: backend offline suite 273 passed; full integration suite 107 passed; Ruff and mypy clean for 88 application source files; frontend lint/typecheck clean, Vitest 43 tests across 19 files, Playwright e2e 10 tests across desktop and mobile projects, and `next build` succeeded. The committed implementation was merged locally to `main`; the WP6 exit gate is satisfied and WP7 is Ready for planning.
...
**Status:** Ready for planning — WP3 and WP6 are Complete.

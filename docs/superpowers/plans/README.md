...
| 4 | WP4 Read APIs and rule lifecycle | In progress | WP1 and WP3 | Approved [specification](../specs/2026-07-21-wp4-read-apis-design.md) and [implementation plan](2026-07-21-wp4-read-apis.md) |
...
WP4 is In progress on branch `codex/wp4-read-apis` (not yet merged). The nine read routes (JD-scoped ranked candidate list, flat candidate list, candidate detail with audited PII decrypt, score detail with evidence, audited raw-file presigned download, JD list/detail, rule-version list, and rule-version diff), their response models, and the read-service layer are implemented per the [WP4 design](../specs/2026-07-21-wp4-read-apis-design.md) and [implementation plan](2026-07-21-wp4-read-apis.md). The local gate through Task 6 passed: 260 offline tests; the full integration suite, including the new OpenAPI contract test asserting all nine routes and strengthened rule-version `is_active`/`published_at`-ordering/unknown-version-404 assertions, 79 passed against real PostgreSQL/Redis/MinIO; Ruff and mypy clean for 79 application source files. WP4 is not yet Complete and WP5 is not yet Ready for planning — both remain gated on hosted CI, per Task 7 of the WP4 plan.

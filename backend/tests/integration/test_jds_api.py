...
async def test_rule_versions_and_diff(client, db_session, auth_headers):
    ...
    # v2 was published later than v1 and is the JD's active_rule_version_id,
    # so it must sort first (published_at DESC) and be the only active item.
    items = body["items"]
    assert [i["version"] for i in items] == ["v2", "v1"]
    by_version = {i["version"]: i for i in items}
    ...
    missing = await client.get(
        f"/api/v1/jds/{jd.code}/rule-versions/v1/diff/NOPE", headers=await auth_headers("hr")
    )

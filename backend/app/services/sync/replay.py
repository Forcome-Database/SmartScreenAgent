    ...
    *,
    ...
    for index, (row_id, external_id, stored_sha) in enumerate(pending):
        try:
            ...
            # A fact about the adapter, not about this item, so the remaining
            # rows are in the same position and asking each one would be
            # pointless. Nothing is recorded: no attempt is spent on a question
            # that was never put to the source.
        ...
        async with session_factory() as session:
            ...
            await session.commit()
        if seen:
            # Something already ingested these exact bytes for this candidate —
            # most likely the runner, which re-lists a newest-in-window item on
            # every run. The row we selected is `failed`, and the identity
            # triple is unique, so it cannot BE that ingested row: it is a
            # superseded record of an attempt that has since succeeded.
            # Deleting it is safe and necessary — left in place it matches the
            # predicate on every future sweep and re-downloads the same
            # attachment until it exhausts.
            superseded += 1
        ...
        try:
            async with session_factory() as session:
                job, created = await ingest_upload(
                    ...
                    source=source,
                    # The LEDGER's id, not the described item's. They are the
                    # same for any correct adapter, but this is the id the row
                    # was selected by and the id the attempt bound is keyed to,
                    # so the job and the ledger cannot disagree about which
                    # candidate this is even if `describe` echoes back a
                    # different one.
                ...
                if fetched.sha256 != stored_sha:
                    ...
                    # almost always `UNKNOWN_SHA256`, the placeholder a fetch
                    # failure leaves behind because the bytes were never seen.
                    # `record_item` above therefore wrote a new row under the
                    # real hash and left this one `failed` and still inside the
                    # bound. Its meaning ("this id has no fetched content") has
                    # just stopped being true, and the durable history of the
                    # failure lives in the `resume_sync_*` audit trail, not in
                    # a dedupe ledger. When the hashes match there is nothing
                    # to discard: `record_item` updated this very row.
                    await session.execute(

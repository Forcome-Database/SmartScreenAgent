...
WP6a 已通过本地全量门禁（后端 offline 264/integration 96/ruff/mypy，前端
vitest 36/Playwright e2e 6/build）与托管 CI（[verify run 29899061689](https://github.com/Forcome-Database/SmartScreenAgent/actions/runs/29899061689)，PR #6），已标记为 Complete。

## WP6b — 黄金集与基线指标

WP6b 在 WP6a 之上加入一份独立于评分本身的**权威金标签**（ground truth）与一份
只读的**基线指标**报表，用于客观衡量当前 AI 推进/淘汰判定与金标签的一致程度
（precision/recall/F1/accuracy、混淆矩阵）。WP6b 与 WP6a 的区别：WP6a 记录的
是"该复核人是否认同 AI"（复核人可能是错的）；WP6b 记录的是一份经过策展的
**唯一权威真值**，衡量 AI 是否"客观正确"。WP6b 不改变评分行为，也不写回
`scores` 表——金标签是独立记录。

**数据模型与约束**：复用既有的 `golden_set` 表（`id`、`candidate_id`、
`jd_id`、`label`、`imported_at`、`imported_by_user_id`，`(candidate_id,
jd_id)` 唯一约束 `uq_golden_set_cand_jd`），新增迁移
[`f412481450cf_wp6b_golden_set_label_check.py`](migrations/versions/f412481450cf_wp6b_golden_set_label_check.py)
（revises WP6a 的 `1e9b39dbf340`）添加 `label` 的 CHECK 约束，限定为
`advance`/`reject`/`borderline`。

**AI 判定与金标签映射**：AI 判定拒绝当且仅当 `score.grade == "rejected"`
（与 WP6a 相同约定）；二分类指标以 `advance` 为正类；`borderline` 金标签
排除在混淆矩阵之外（计入 `borderline_excluded`）；一个 `(candidate, JD)`
若有多条评分（不同规则版本），取 `created_at` 最新的一条；没有对应评分的
金标签计入 `uncovered`，同样排除在指标之外。

**接口**：

- `POST /api/v1/golden-set/import` —— 角色 `hr_lead`/`admin`；上传含
  `candidate_id, jd_code, label` 三列的 CSV，逐行校验（未知 `jd_code`/
  `candidate_id`、非法 `label`、格式错误行各自记为行级错误，不影响其余行），
  按 `uq_golden_set_cand_jd` upsert（同文件内重复 `(candidate_id, jd_code)`
  以最后一次为准）；单次导入行数上限 `GOLDEN_IMPORT_MAX_ROWS`（默认
  5000），超限返回 `422 golden_import_too_large`；整份文件无法解析返回
  `422 invalid_csv`。响应 `{total, created, updated, errors:
  [{row, candidate_id, jd_code, reason}]}`，不含候选人 PII。
- `GET /api/v1/golden-set?jd_code=&page=&page_size=` —— 角色
  `hr`/`hr_lead`/`admin`；按 `imported_at` 降序分页列出
  `{id, candidate_id, jd_code, label, imported_at,
  imported_by_display_name}`，不含 PII。
- `GET /api/v1/golden-set/metrics?jd_code=` —— 角色
  `hr`/`hr_lead`/`admin`；实时计算 `overall` 与 `by_jd`（各含
  `labeled_total, scored, uncovered, borderline_excluded,
  confusion: {tp, fp, tn, fn}, precision, recall, f1, accuracy`，分母为 0 时
  对应指标为 `null`），不含 PII。

**前端**：新增 `/golden-set` 页面（`frontend/src/app/(app)/golden-set/page.tsx`
+ `frontend/src/components/golden-set-view.tsx`）—— CSV 导入控件仅
`hr_lead`/`admin` 可见，导入结果摘要（created/updated/错误行）与分页列表；
新增 `/reports/baseline` 基线指标报表页
（`frontend/src/app/(app)/reports/baseline/page.tsx`）展示总体与按 JD 的
precision/recall/F1，以及混淆矩阵/覆盖率；导航栏新增"黄金集"与"基线指标"
入口。两者复用既有的 `apiGet`（`golden-set` 导入复用 WP5 上传的 FormData
转发 BFF 模式）与 zod 响应校验（`GoldenImportResult`/`GoldenSetList`/
`GoldenMetricsReport`，见 `frontend/src/lib/schemas.ts`）。

**测试**：后端单元覆盖 CSV 解析/校验与指标数学（混淆矩阵四象限、
precision/recall/F1/accuracy 含零分母 → null、borderline 排除、uncovered
统计）；集成测试覆盖导入 upsert/唯一约束更新/各类行错误、权限矩阵
（无 token → 401、`hr` 导入 → 403）、指标聚合正确性（含"最新评分"选取与
按 JD 拆分）与 Alembic 升降级往返。前端单元测试覆盖导入摘要渲染与报表页
组件；新增的 Playwright e2e
[`frontend/e2e/golden-set.spec.ts`](frontend/e2e/golden-set.spec.ts) 用桩接的
`/api/proxy/api/v1/golden-set/metrics` 响应驱动 `/reports/baseline`，断言
总体 precision 正确渲染（desktop + mobile 两套 project）。设计文档：
[`docs/superpowers/specs/2026-07-22-wp6b-golden-set-baseline-metrics-design.md`](docs/superpowers/specs/2026-07-22-wp6b-golden-set-baseline-metrics-design.md)；
实施计划：
[`docs/superpowers/plans/2026-07-22-wp6b-golden-set-baseline-metrics.md`](docs/superpowers/plans/2026-07-22-wp6b-golden-set-baseline-metrics.md)。

WP6b 本地全量门禁已通过（后端 offline 268/integration 101/ruff/mypy，前端
vitest 39/Playwright e2e 8/build），尚未标记为 Complete（等待托管 CI）。
